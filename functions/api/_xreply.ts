...
/**
 * 既定のミュート語（本文にこれを含む候補は返信先にしない）。
 *
 * 出典は 2026-08-14 の実 digest 49 件の目視分類。ここに置くのは
 * **「その語を含む投稿は、返信しても会話にならない」と言い切れるもの**だけ。
 * 1 語で投稿を丸ごと落とすため、良い投稿を巻き込む語は入れない
 * （例: 「テスター募集」は個人開発者の正当な発信なので入れていない）。
 *
 * 検索クエリの除外語（`-まとめ` 等）と役割が違う: あちらは検索結果ごと
 * 削るので取りこぼしが読めない。こちらは取得後に落とすので、何が落ちたかを
 * 件数で追える。
 */
export const DEFAULT_MUTED_TERMS: string[] = [
  // 自動投稿の集約 bot（毎日同じ体裁で流れる）
  "techfeedまとめ",
  "noteに書いたよ",
  // #個人開発 を付けているが開発の話ではない AI 生成画像の投稿
  "#aiart",
  // 拡散懸賞・相互フォロー系（返信は宣伝の踏み台にしかならない）
  "フォロー&リポスト",
  "フォロー＆リポスト",
  "プレゼント企画",
];
