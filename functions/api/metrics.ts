...
// extended with user.fields=public_metrics) + the /try KV counters. Gated by
// PUBLISH_TOKEN, exactly like /api/publish.
//
// The waitlist deliberately has NO field to read: the LP waitlist is a Tally.so
// embed (form dWQlbq) with no repo-side backend/DB/KV (docs/18 §9.2), so its
// count is filled by hand from the Tally dashboard.
//
// Consume with scripts/metrics-snapshot.mjs, which prints a ready-to-paste
// docs/METRICS_LOG.md row.
...
async function handleGet(context: PagesContext): Promise<Response> {
  ...
  const [publishCountToday, spendUsdToday] = await Promise.all([
  ...
  return json(
    {
      ...snapshot,
      // Ready-to-paste docs/METRICS_LOG.md row (waitlist is a manual cell).
      logLine: buildMetricsLogLine(snapshot),
      notes: [
        "waitlist has no repo-side source (Tally.so form dWQlbq) — fill from the Tally dashboard, see docs/18 §9.2",
